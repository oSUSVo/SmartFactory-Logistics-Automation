...
class Hcr3Action : public rclcpp::Node {
public:
    using IntAct = custom_interfaces::action::IntAct;
    using GoalHandle = rclcpp_action::ServerGoalHandle<IntAct>;

    explicit Hcr3Action(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
    : Node("hcr3_node", options) {
        this->server_ = rclcpp_action::create_server<IntAct>(
            this,
            "/hcr3",
            std::bind(&Hcr3Action::handle_goal, this, _1, _2),
            std::bind(&Hcr3Action::handle_cancel, this, _1),
            std::bind(&Hcr3Action::handle_accepted, this, _1)
        );

        publisher_ = this->create_publisher<std_msgs::msg::Int32>("/hcr3_pub", 10);
        subscription_ = this->create_subscription<std_msgs::msg::Int32>(
            "/hcr3_sub", 10, std::bind(&Hcr3Action::receiveTopic, this, _1)
        );
...
void Hcr3Action::receiveTopic(const std_msgs::msg::Int32::SharedPtr msg) {
    act_reseult_data_ = msg->data;
    act_result_flag_ = true;
}

rclcpp_action::GoalResponse Hcr3Action::handle_goal(const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const IntAct::Goal> goal) {
    RCLCPP_INFO(this->get_logger(), "Received goal request with ID: %d", goal->goal_num);
    (void)uuid;
    return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse Hcr3Action::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle) {
    RCLCPP_INFO(this->get_logger(), "Received request to cancel goal");
    (void)goal_handle;
    return rclcpp_action::CancelResponse::ACCEPT;
}

void Hcr3Action::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle) {
    std::thread([this, goal_handle]() {
        this->execute(goal_handle);
    }).detach();
}

void Hcr3Action::execute(const std::shared_ptr<GoalHandle> goal_handle) {
    act_result_flag_ = false;
    act_reseult_data_ = 0;
